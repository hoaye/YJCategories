...
static YJ_HTMLEscapeMap yj_gAsciiHTMLEscapeMap[] = {
...
static YJ_HTMLEscapeMap yj_gUnicodeHTMLEscapeMap[] = {
...
static int YJ_EscapeMapCompare(const void *ucharVoid, const void *mapVoid) {
    const unichar *uchar = (const unichar*)ucharVoid;
    const YJ_HTMLEscapeMap *map = (const YJ_HTMLEscapeMap*)mapVoid;
    int val;
    if (*uchar > map->uchar) {
        val = 1;
...
- (NSString *)yj_stringByEscapingHTMLUsingTable:(YJ_HTMLEscapeMap*)table
    ...
    for (NSUInteger i = 0; i < length; ++i) {
        YJ_HTMLEscapeMap *val = bsearch(&buffer[i], table,
                                        size / sizeof(YJ_HTMLEscapeMap),
                                        sizeof(YJ_HTMLEscapeMap), YJ_EscapeMapCompare);
...
- (NSString *)yj_stringByEscapingForHTML {
    return [self yj_stringByEscapingHTMLUsingTable:yj_gUnicodeHTMLEscapeMap
                                            ofSize:sizeof(yj_gUnicodeHTMLEscapeMap)
                                   escapingUnicode:NO];
} // gtm_stringByEscapingHTML

- (NSString *)yj_stringByEscapingForAsciiHTML {
    return [self yj_stringByEscapingHTMLUsingTable:yj_gAsciiHTMLEscapeMap
                                            ofSize:sizeof(yj_gAsciiHTMLEscapeMap)
                                   escapingUnicode:YES];
    ...
    do {
        ...
        if (length > 3 && length < 11) {
            if ([escapeString characterAtIndex:1] == '#') {
                ...
                // "standard" sequences
                for (unsigned i = 0; i < sizeof(yj_gAsciiHTMLEscapeMap) / sizeof(YJ_HTMLEscapeMap); ++i) {
                    if ([escapeString isEqualToString:yj_gAsciiHTMLEscapeMap[i].escapeSequence]) {
                        [finalString replaceCharactersInRange:escapeRange withString:[NSString stringWithCharacters:&yj_gAsciiHTMLEscapeMap[i].uchar length:1]];
                        break;
                    }
                }
            }
...
- (NSString *)yj_stringByStrippingTags {
    @autoreleasepool {
        ...
        finalString = [result yj_stringByRemovingNewLinesAndWhitespace];
...
- (NSString *)yj_stringByConvertingHTMLToPlainText {
    @autoreleasepool {
        ...
        NSString *retString = [result yj_stringByUnescapingFromHTML];
